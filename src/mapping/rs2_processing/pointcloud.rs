...
use anyhow::Error;
use realsense_rust::check_rs2_error;
use realsense_rust::frame::{DepthFrame, FrameEx, PointsFrame};
use realsense_rust::pipeline::PipelineConstructionError;
use realsense_rust::stream_profile::DataError;
use realsense_sys::{rs2_create_frame_queue, rs2_create_pointcloud, rs2_delete_frame_queue, rs2_delete_processing_block, rs2_error, rs2_frame_queue, rs2_process_frame, rs2_processing_block, rs2_start_processing_queue, rs2_wait_for_frame};
...
impl PointCloudProcBlock {
    ...
    pub fn wait(&mut self, timeout: Duration) -> Result<PointsFrame, DataError>{
        unsafe{
            ...
            let point_frame = rs2_wait_for_frame(self.processing_queue.as_ptr(), timeout_millis, &mut err);
