...
impl Map {
    ...
    pub fn disp_map(&mut self){
        ...
        let cell_height: f32 = (grid_disp_height - ((self.height as f32 + 2.0) * LINE_THICKNESS)) / (self.height as f32);
        ...
        while !rl.window_should_close() {
            ...
            d.draw_rectangle_lines_ex((Rectangle::new((WINDOW_WIDTH_START), WINDOW_HEIGHT_START, grid_disp_width, grid_disp_height)), LINE_THICKNESS,Color::BLACK);

            //Draw the grid lines
            for i in 1..self.width{

                let curr_x = (WINDOW_WIDTH_START + LINE_THICKNESS) + (i as f32 * (cell_width + LINE_THICKNESS));

                d.draw_line_ex(Vector2::new(curr_x, WINDOW_HEIGHT_START), Vector2::new(curr_x , WINDOW_HEIGHT_END), LINE_THICKNESS, Color::BLACK);
            }
            for i in 1..self.height{

                let curr_y = (WINDOW_HEIGHT_START + LINE_THICKNESS) + (i as f32 * (cell_height + LINE_THICKNESS));

                d.draw_line_ex(Vector2::new(WINDOW_WIDTH_START, curr_y), Vector2::new(WINDOW_WIDTH_END , curr_y), LINE_THICKNESS, Color::BLACK);
